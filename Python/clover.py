...
class RTError(Error):
	...
	def generate_traceback(self):
		...
		while ctx:
			result = f"\tFile {pos.fn}, line {str(pos.ln + 1)}, column {str(pos.col)}, in {ctx.display_name}\n" + result
	...
	def make_tokens(self):
		...
		while self.current_char != None:
			if self.current_char in " \t":
				...
				self.advance()
				return [], IllegalCharError(pos_start, self.pos, "'" + char + "' not recognized!")
...
class Parser:
	...
	def statement(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'RETURN', 'CONTINUE', 'BREAK', 'VAR', 'IF', 'FOR', 'WHILE', 'func', int, float, identifier, '+', '-', '(', '[' or 'NOT'"))
	...
	def comp_expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected int, float, identifier, '+', '-', '(', '[', NOT"))
...
class List(Value):
	...
	def __str__(self):
		#grabs all the elements, puts in string
...
class BuiltInFunction(BaseFunction):
	...
	def execute_pop(self, exec_ctx):
		...
		if not isinstance(index, Number):
			return RTResult().failure(RTError(self.pos_start, self.pos_end, "First argument must be a number", exec_ctx))

		try:
			element = list_.elements.pop(index.value)
		except:
			return RTResult().failure(RTError(self.pos_start, self.pos_end, "Element at this index could not be removed because index is out of bounds", exec_ctx))
	...
	def execute_run(self, exec_ctx):
		...
		if not ".clvr" in fn:
			return RTResult().failure(RTError(self.pos_start, self.pos_end, "Failed to load script,\nfile must be of type '.clvr'", exec_ctx))
